...
int* costruiscoPercorsoAggiunto(int** matrice, int** hasse, int righe, int colonne, int source, int nodoInterno, int sink);
...
//cout<<".....in realizza specie, realizzo specie ..................."<<specie<<endl;
 ...
 //       cout<<endl;
        ...
        //SE TUTTI  I CARATTERI DI UNA SPECIE SONO GIà STATI REALIZZATI --> STOP
        conta=0;
        for(i=0; i<colonne; i++){
...
//elimina eventuali righe uguali e righe dove nessun carattere è da attivare
void compattaIndottaMassimali(){
        int i,j, nr, i_matrice;
        int* copia;
        int nUni;
...
void trovaPercorsi(int** matrice, int** hasse, int** grbMA, int righe, int colonne, int* componentiRiga){ //matrice di massimali e attivi
...
cout<<"------------------------------------------------------------------------------------------------"<<endl;
             ...
             //se nessun percorso è safe da entrambe le parti --> no soluzione
              if(nSafe==0){
...
//se la sorgente è la stessa in tutti i percorsi safe, realizzo quella sorgente
else if(sorgenteUnica(percorsi, safe, safeGRB)){
    cout<<"la sorgente safe è sempre la stessa, realizzo quella!!!"<<endl;
    for(i=0; i<1000; i++){
                            if(stop==0){
                                if((safe[i]==1)&(safeGRB[i]==1)){
                                        stop=1;
...
//            int specieStar; //specie per cui sto cercando se esiste s*
        ...
        //se non ho specie star, realizzo la specie safe con più car attivi
             //    else if(countStar==0){
                       // cout<<"nessuna specie corrispondente a specie di matrice iniziale --> realizzo la specie safe con più car attivi"<<endl;
             cout<<"nessuna specie corrispondente a specie di matrice iniziale -- cerco specie in matrice iniziale con TUTTI i car attivi + car sorgente safe"<<endl;
  ...
  /*                //costruisco le specie corrispondenti alle sorgenti, senza gli attivi
        ...
        for(i=0; i<1000; i++){
         ...
         for(i=0; i<1000; i++){
                   // if(stop=0){
                        if((safe[i]==1)&(safeGRB[i]==1)){
                    //verifico se la specie costruita è contenuta nella matrice iniziale
                 ...
                 indice=indiceMax;
                  cout<<"sorgente safe da realizzare: "<<indice<<endl;
                    ...
                    cout<<"ATTENZIONE perché il massimo ha doppioni"<<endl;
       ...
       for(i=0; i<1000; i++){
        ...
        //per ogni sorgente safe, conto il numero di inclusioni nella matrice iniziale
                 ...
                 if(uguali==0){
                 cout<<"SECONDO: realizzo la sorgente inclusa il maggior numero di volte nelle specie della matrice iniziale"<<endl;
                 indice=indiceMax;
                  cout<<"SECONDO: sorgente safe da realizzare: "<<indice<<endl;
                    ...
                    cout<<"SECONDO: ATTENZIONE perché il massimo ha doppioni"<<endl;
       ...
       for(i=0; i<1000; i++){
        ...
        //per ogni sorgente safe, conto il numero di inclusioni nella matrice iniziale
               ...
               //  if(uguali==0){
                 cout<<"TERZO: realizzo la sorgente inclusa il maggior numero di volte nelle specie della matrice iniziale"<<endl;
                 indice=indiceMax;
...
//cout<<endl<<endl<<endl;
        ...
        //    }
                  //REGOLA 1: se sono in relazione di inclusione, scegli la più piccola
                  //prendi la specie più piccola; se questa è inclusa nelle altre, realizza questa
...
cout<<"la specie è inclusa in tutte le altre"<<endl;
return 1;
}
...
//determina se un carattere è attivo nella matrice considerata
int attivo(int carattere, int** matrice, int righe){
    int i;
    for(i=0; i<righe; i++){
    ...
    for(s=0; s<righeO; s++){
      ...
      for(i=0; i<colonne; i++){
        if(matrice[s1][i]==1){
            ...
            else ok[corrispondenzaMC[i]]=1; //car a 1 in s1 che è a 1 in s*
        }
    }
//if(sstar[s]==0)
     ...
     if(sstar==1){
        ...
        for(i=0; i<colonneO; i++){
            if(((matriceO[s][i]==1) & (car_attivi[i]==1))) ok[i]=1; //car a 1 in s* che è attivo in GRB
        }

      /*  for(i=0; i<colonneO; i++){
            if(((matriceO[s][i]==1) & (car_attivi[i]==0) & (nonOverlap(i)))) ok[i]=1; //car a 1 in s* che è attivo in GRB
        }*/
     }
...
//conta il numero di volte che la specie in ingresso è contenuta nelle specie della matrice iniziale
int contaInclusioni(int* specie){
    int i,j;
    int inclusioni;
    inclusioni=0;
...
for(i=0; i<righeO; i++){
        if(SR[i]==1){  //se la specie è realizzabile, ovvero non ha coppie di caratteri in conflitto
...
void copiaSoluzione(int* soluzione,int indice,int specie, int** matrice, int colonne){  //matrice di soli massimali + attivi
        int i;
