...
//	sigma.resize(numofVertices+1);
    ...
    for (unsigned int j = 1; j < numofCliques +
                                 1; ++j) {                // for j=1 to k (k was a mistake in the original algorithm in the paper!)
        ...
        for (unsigned int ind = 0; ind < numofCliques; ++ind) {
            ...
            if (isLabelGreaterThan(sparseCliques[ind].label, (candidateCliqueIndex == -1 ? sparseCliques[ind].label
                                                                                         : sparseCliques[candidateCliqueIndex].label))) {
        ...
        while (sparseCliques[candidateCliqueIndex].containsUnnumberedOneEntry(numberedVertices,
                                                                              x)) {    //note: x is passed by reference!
            numberedVertices.push_back(x);    //number the vertex (by pushing it to a special container)
//			sigma[i] = x;
            Y[x] = j;
            for (unsigned int index = 0; index <
                                         sparseCliques.size(); ++index) {    //for each unnumbered column C' that contains x as one entry
    ...
    while (L.size() < numofCliques) {        //while there exists a non-singleton class Lc in L
        if (pivots.empty()) {    //pivots can be empty several times, not just only on the first run!
            ...
            while (pivots.size() > 0) {
                ...
                for (unsigned int index = 0; index < numofCliques; ++index) {    //run over all the cliques
                    if (sparseCliques[index].containsXasOneEntry(
                            pivots.top())) {        //if a clique contains vertex x (i.e. pivots.top())
                        ...
                        for (unsigned int classIndex = 0;
                             classIndex < L.size(); ++classIndex) {    //run over all the classes in container L
                ...
                L.insert(L.begin() + Lb,
                         LbIntersectionWithC);    //insert the new class of cliques removed from Lb right before Lb (do this first so La index will not change)
                L.insert(L.begin() + La + 1,
                         LaIntersectionWithC);    //insert the new class of cliques removed from La right after La (do this after Lb so indices will not change)
        ...
        for (unsigned int i = 0; i < treeParentArray.size(); ++i) {
            ...
            if (
                ...
                for (unsigned int index = 0; index <
                                             sparseCliques[i].getVertices().size(); ++index) {    //run over all of the vertices of the clique Ci
                    if (sparseCliques[treeParentArray[i]].containsXasOneEntry(
                            sparseCliques[i].getVertices()[index])) {    //if Cj also contains this vertex
