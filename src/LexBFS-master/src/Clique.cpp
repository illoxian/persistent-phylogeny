...
bool Clique::containsUnnumberedOneEntry(vector<int> numberedVerticesOfAClique, int &x) {
    ...
    for (unsigned int index = 0; index < vertices.size(); ++index) {    //run over all the vertices of this Clique
        ...
        for (unsigned int j = 0; j <
                                 numberedVerticesOfAClique.size(); ++j) {    //try to find a matching vertex in the numberedVerticesOfAClique
            if (vertices[index] - numberedVerticesOfAClique[j] ==
                0) {        //vertices[index] contains 1 entries, and -1 means unnumbered
                isFound = true;
                break;
