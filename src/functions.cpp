...
/** 
 * 
 * @brief additional implementation
 * 
 * @author Mattia Sgro
 * 
 */
...
std::list<SignedCharacter> ppr_general(RBGraph &g) //non const perche dev'essere agigornato
{
  ...
  while (!is_empty(g))
  {
    ppp_maximal_reducible_graphs(g);
    is_2_solvable(sources, gm);
    // Computed G-Skeleton
    // if Gm is 2-solvable
...
std::list<SignedCharacter> source_2_solvable(RBGraph &g)
{
  ...
  if (source_size == 1)
  {
    ...
    extension = get_extension(*get_sources(g).begin(), gmax, gmin);
  ...
  return nullptr; // returning an empty set if either of the 2if statement is taken
}
